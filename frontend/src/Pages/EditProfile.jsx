...
const EditProfile = () => {
    ...
    const formSubmit = (e) => {
        ...
        if(formData.password &&  formData.DOB && formData.address && formData.fullName && formData.email){
            toggleLoading(true);
    ...
    const handleUploadImage = ()=>{
        ...
        if(formData){
            uploadProfilePic(userId, formData).then((resp)=>{
                ...
                if(resp.message=== "ok"){
                    localStorage.setItem("imageUrl",base+'/'+ resp.imageUrl);
  ...
  return (
    <>
        ...
        <form className=" bg-white shadow-lg rounded-lg my-15 mx-60 p-4 grid grid-cols-2 gap-6 mb-10">
            <div>
            <label htmlFor="fullName" className=" mb-1 ml-1 block font-bold text-gray-600 ">Name
                <span className="text-red-500">&nbsp;*</span>
            </label>
            <input type="text" id="fullName" className="w-full border rounded-md py-2 px-3 focus:outline-none  focus:border-red-200" name="fullName" placeholder={userData?.fullName} value={formData.fullName} onChange={handleFormChange} required />
            </div>
            ...
            <input type="email" id="email" className="w-full border rounded-md py-2 px-3 focus:outline-none  focus:border-red-200" name="email" placeholder={userData?.email} value={formData.email} onChange={handleFormChange} required />
            </div>
            ...
            <input type="number" id="contact" className="w-full border rounded-md py-2 px-3 focus:outline-none  focus:border-red-200" name="contact" placeholder={userData?.contact}  value={formData.contact} onChange={handleFormChange} required />
            </div>
            ...
            <input type="text" id="address" className="w-full border rounded-md py-2 px-3 focus:outline-none  focus:border-red-200" name="address" placeholder={userData?.address}  value={formData.address} onChange={handleFormChange} required />
            </div>
